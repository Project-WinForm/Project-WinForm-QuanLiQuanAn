...
namespace tieuluan
{
    public partial class Form1 : Form
    {
        ...
        private object _tableOrders;

        public Form1()
        {
            InitializeComponent();

            // ĐÃ CẬP NHẬT TÊN NÚT MỚI
        ...
        {
            if (_selectedTableBtn != null && !_orderService.HasOrder(_selectedTableBtn.Tag.ToString()))
        ...
        private void btnAddFood_Click(object sender, EventArgs e)
        {
            ...
            // ĐÃ SỬA TÊN nmrSoluong
            if (string.IsNullOrEmpty(_currentTableName) || string.IsNullOrEmpty(_currentFoodName)) return;

            int quantity = (int)nmrSoluong.Value;

            // Gọi Service xử lý
            _orderService.AddFood(_currentTableName, _currentFoodName, _currentFoodPrice, quantity);

            // Cập nhật giao diện nút bàn
        ...
        private void btnRemove_Click(object sender, EventArgs e)
        {
            if (lsvHoadon.SelectedItems.Count > 0)
            {
                ...
                // Gọi Service xóa
                _orderService.RemoveFood(_currentTableName, foodName);

                // Kiểm tra xem bàn còn món nào không để đổi màu nút bàn
                if (!_orderService.HasOrder(_currentTableName))
                {
        ...
        {
            if (!_orderService.HasOrder(_currentTableName)) return;

            if (MessageBox.Show($"Thanh toán cho {_currentTableName}?", "Xác nhận", MessageBoxButtons.YesNo) == DialogResult.Yes)
            {
                // Gọi Service dọn dẹp bill
                _orderService.Checkout(_currentTableName);
        ...
        private void ShowBill(string tableName)
        {
            ...
            // Lấy dữ liệu từ Service thay vì Dictionary cũ
            List<OrderItem> currentBill = _orderService.GetBill(tableName);
